...
echo -e "${YELLOW}PySpider Redisモード起動スクリプト${NC}"
echo "=================================="
...
# Redisポート（6379）も確認
echo -e "${YELLOW}Redisポート（6379）を使用しているプロセスを確認中...${NC}"
REDIS_PIDS=$(lsof -i :6379 -t 2>/dev/null)

if [ -n "$REDIS_PIDS" ]; then
    echo -e "${GREEN}Redisサーバーが既に実行中です${NC}"
else
    echo -e "${YELLOW}Redisサーバーを起動しています...${NC}"
    # Redisサーバーが起動していない場合は起動
    redis-server &
    REDIS_PID=$!
    sleep 2
    echo -e "${GREEN}Redisサーバーを起動しました${NC}"
fi
...
# PySpiderをRedisモードで起動
echo -e "${YELLOW}PySpiderをRedisモードで起動しています...${NC}"
# Python 3.13対応のコマンド
python -m pyspider.run -c redis_config.json all &
PYSPIDER_PID=$!

# 起動完了メッセージ
echo -e "${GREEN}PySpiderをRedisモードで起動しました${NC}"
echo "=================================="
