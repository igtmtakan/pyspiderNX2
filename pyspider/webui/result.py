...
@app.route('/results-v2')
@app.route('/result-v2/<project>')
def result_v2(project=None):
    resultdb = app.config['resultdb']
    if project is None:
        project = request.args.get('project')
    offset = int(request.args.get('offset', 0))
    ...
    count = resultdb.count(project)
    results = list(resultdb.select(project, offset=offset, limit=limit))
    common_fields, results = result_dump.result_formater(results)

    return render_template(
        "result-v2.html", count=count, results=results,
...
def dump_result(project, _format):
    ...
    if _format == 'json':
        ...
        return Response(result_dump.dump_as_xml(results),
