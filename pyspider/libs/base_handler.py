...
# Python 3.10+ compatibility: removed six dependency
...
class BaseHandler(object, metaclass=BaseHandlerMeta):
    """
    ...
    def _crawl(self, url, **kwargs):
        ...
        if kwargs.get('callback'):
            ...
            elif callable(callback) and getattr(callback, '__self__', None) is self:
                func = callback
            ...
            if hasattr(func, '_config'):
                # Python 3.13 compatibility: use items() instead of iteritems
                for k, v in func._config.items():
    ...
    def crawl(self, url, **kwargs):
        ...
        if isinstance(url, str) and url.startswith('curl '):
            curl_kwargs = curl_to_arguments(url)
            url = curl_kwargs.pop('urls')
            # Python 3.13 compatibility: use items() instead of iteritems
            for k, v in curl_kwargs.items():
                kwargs.setdefault(k, v)
